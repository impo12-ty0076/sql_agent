...
async def get_result(
    ...
    This endpoint retrieves the full query result data for the specified result ID.
    """
    try:
        # Get current user ID
...
async def get_paginated_result(
    ...
    """
    Get a paginated query result
    
    This endpoint retrieves a paginated subset of the query result data for the specified result ID.
    """
    try:
        # Get current user ID
        user_id = await get_current_user_id(token)
...
async def generate_result_summary(
    ...
    This endpoint generates a natural language summary of the query result data.
    """
    try:
        # Get current user ID
        user_id = await get_current_user_id(token)
...
async def create_report_from_result(
    ...
    """
    Create a report from a query result
    
    This endpoint creates a report from the query result data. The report generation
    is performed as a background task, and the status can be checked using the
    /report-status/{task_id} endpoint.
    """
    try:
        # Get current user ID
        user_id = await get_current_user_id(token)
...
async def get_report_generation_status(
    ...
    This endpoint retrieves the status of a report generation task.
    """
    try:
        # Get current user ID
        user_id = await get_current_user_id(token)
