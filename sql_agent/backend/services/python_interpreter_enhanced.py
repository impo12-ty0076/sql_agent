"""
Enhanced Python Code Execution Service with Data Analysis Integration

This service extends the Python interpreter with integrated data analysis capabilities
...
from .data_analysis import (
    DataLoader, DataPreprocessor, StatisticalAnalysis, DataAnalysisService, AnalysisResult
)
...
class EnhancedPythonInterpreterService:
    """Service for executing Python code with data analysis capabilities"""
    ...
    def _setup_execution_environment(self) -> Dict[str, Any]:
        """Setup the execution environment with allowed modules and data analysis utilities"""
        ...
        # Add Plotly chart generator if available
        if HAS_PLOTLY:
            safe_globals['PlotlyChartGenerator'] = PlotlyChartGenerator
        
        return safe_globals
    
    def _capture_plots(self) -> list:
        """Capture matplotlib and plotly plots as base64 encoded images"""
        plots = []
        ...
        for fig_num in plt.get_fignums():
            ...
            # Create visualization using our service
        ...
        # Try to capture Plotly figures if available
        # Note: This requires the user code to store Plotly figures in a specific variable
        # pattern like 'plotly_fig', 'plotly_figure', etc.
        if HAS_PLOTLY and 'exec_locals' in dir(self):
            for var_name, var_value in self.exec_locals.items():
                if isinstance(var_value, (go.Figure, px.Figure)) or (
                    hasattr(var_value, 'to_dict') and 
                    callable(var_value.to_dict) and
                    'data' in var_value.to_dict()
                ):
                    try:
                        # Create visualization using our service
                        vis = VisualizationService.create_plotly_visualization(
                            title=var_name,
                            fig=var_value,
                            format='png'
                        )
                        ...
                        print(f"Error capturing Plotly figure {var_name}: {str(e)}")
    ...
    def _analyze_dataframes(self, variables: Dict[str, Any]) -> Dict[str, Any]:
        """
        Analyze DataFrames in the execution variables
        
        Args:
            variables: Dictionary of variables from execution
            
        Returns:
            Dictionary of analysis results for each DataFrame
        """
        analysis_results = {}
        
        # Find all DataFrames in variables
        for var_name, var_value in variables.items():
            if isinstance(var_value, pd.DataFrame) and not var_value.empty:
                # Skip DataFrames that are too large
                if var_value.shape[0] * var_value.shape[1] > 1000000:  # ~1M cells
                    analysis_results[var_name] = {
                        "skipped": True,
                        "reason": "DataFrame too large for automatic analysis",
                ...
                # Perform analysis
                try:
                    result = DataAnalysisService.analyze_dataframe(var_value)
                    if result.success:
                        # Generate insights
                        insights = DataAnalysisService.generate_insights(var_value)
    ...
    def _execute_code_thread(self, code: str, context: ExecutionContext, 
        ...
        try:
            ...
            try:
                ...
                # Store exec_locals for Plotly figure capture
                ...
                # Analyze DataFrames
                result.analysis_results = self._analyze_dataframes(result.variables)
...
enhanced_python_interpreter_service = EnhancedPythonInterpreterService()