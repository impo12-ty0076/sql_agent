...
describe('HistoryDetail', () => {
  ...
  const mockQueryDetails = {
    ...
    generated_sql: "SELECT SUM(amount) FROM sales WHERE date >= '2025-06-01' AND date <= '2025-06-30'",
    result: {
      columns: ['total_sales'],
      rows: [[1500000]]
    }
  ...
  test('edits and saves notes', () => {
    ...
    const editNotesButton = editButtons.find(button => 
      button.parentElement?.previousElementSibling?.textContent === '메모'
    );
