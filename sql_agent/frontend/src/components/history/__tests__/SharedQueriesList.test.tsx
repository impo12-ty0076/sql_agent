...
describe('SharedQueriesList', () => {
  ...
  it('renders loading state correctly', () => {
    render(
      <Provider store={store}>
        <SharedQueriesList historyId="123" shareLinks={[]} loading={true} error={null} />
      </Provider>
    );
  ...
  it('renders empty state correctly', () => {
    render(
      <Provider store={store}>
        <SharedQueriesList historyId="123" shareLinks={[]} loading={false} error={null} />
      </Provider>
    );
  ...
  it('renders share links correctly', () => {
    ...
    render(
      <Provider store={store}>
        <SharedQueriesList historyId="123" shareLinks={shareLinks} loading={false} error={null} />
      </Provider>
    );
    ...
    expect(
      screen.getByText(/허용된 사용자: user1@example.com, user2@example.com/)
    ).toBeInTheDocument();
  });
  ...
  it('shows delete confirmation dialog when delete button is clicked', () => {
    ...
    expect(
      screen.getByText('이 공유 링크를 삭제하시겠습니까? 이 작업은 취소할 수 없습니다.')
    ).toBeInTheDocument();
  });
