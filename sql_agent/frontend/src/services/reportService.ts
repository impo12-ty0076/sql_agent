...
export const reportService = {
  generateReport:
    (resultId: string, visualizationTypes: string[] = [], includeInsights = true) =>
    async (dispatch: AppDispatch) => {
      dispatch(startReportGeneration());

      try {
        // Start report generation
        const response = await resultAPI.generateReport(
          resultId,
          visualizationTypes,
          includeInsights
        );
  ...
  getReport: (reportId: string) => async (dispatch: AppDispatch) => {
    try {
